SELECT
   ...
   ,t4.project as project
    ...
    AS cofactors
    --    ,SUBSTR(t1.result_alpha, 1,1) MODIFIER
...
WHERE
    ...
    --    AND (NOT REGEXP_LIKE(SUBSTR(t1.result_alpha, 1,1), '[<>~]') 
--    OR NOT REGEXP_LIKE(SUBSTR(t1.result_numeric, 1,1), '[<>~]')) -- 34,283  Rows
UNION ALL
SELECT
   ...
   ,NVL(NVL2(t5.cofactor_1, t5.cofactor_1, NULL) || NVL2(t5.cofactor_2, ', ' || t5.cofactor_2, NULL), '-')
    AS cofactors
    --   ,SUBSTR(to_char(t1.reported_result), 1,1) MODIFIER
    ,nvl2(t5.atp_conc_um, t5.atp_conc_um || 'uM', ' ') as atp_conc_um
...
WHERE
     ...
     AND t1.STATUS = 1
--     AND (REGEXP_LIKE(SUBSTR(t1.reported_result, 1,1), '[0-9]')
--     OR REGEXP_LIKE(SUBSTR(t1.result_numeric, 1,1), '[0-9]'))
;
...
AND (REGEXP_LIKE(SUBSTR(t1.reported_result, 1,1), '[0-9]')
     OR REGEXP_LIKE(SUBSTR(t1.result_numeric, 1,1), '[0-9]'))
--     AND t4.experiment_id = 210644
)
SELECT *
FROM T
PIVOT
(
    AVG(LOG(10, ic50))--, MIN(ic50), STDDEV(ic50)
    FOR (
    COMPOUND_ID 
    )
    IN (
...
select POWER(10, -8.51218951276642424547310444050891653307)* TO_NUMBER('1.0e+09') FROM DUAL;     
     
     
     
     
     
     select * from tm_experiments_PROPS_PIVOT where protocol_id = 501;
     
     select * from SU_PLATE_PROP_PIVOT where experiment_id = 210644;
     
     select * from tm_experiments where experiment_id = 210644;